...
Yes, with an Anthropic API key billed per token. Claude Pro and Max subscriptions only work inside Anthropic's own tools, so they cannot be pointed at OpenCode. Community plugins that work around this exist, but they are against Anthropic's terms.
